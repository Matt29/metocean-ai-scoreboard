...
def test_columns_exactly_as_specified():
    feats = build_features(_baseline(), _series(T0 - pd.Timedelta(hours=24), 25, 1.3), T0)
...
def test_constant_case_errors_and_leads():
    ...
    feats = build_features(_baseline(), obs, T0)
...
def test_empty_obs_history_gives_zeros_not_nan():
    ...
    feats = build_features(_baseline(), empty, T0)
...
def test_no_feature_is_ever_nan_with_gappy_baseline():
    ...
    obs = _series(T0 - pd.Timedelta(hours=24), 25, 1.3)
    feats = build_features(baseline, obs, T0)
    assert not feats.isna().any().any()
...
def test_anti_leak_future_obs_do_not_influence_features():
    ...
    baseline = _baseline()
    clean = build_features(baseline, honest, T0)
    dirty = build_features(baseline, poisoned, T0)
...
def test_anti_leak_last_err_uses_last_obs_at_or_before_t0():
    ...
    feats_late = build_features(_baseline(), obs_late, T0)
...
def test_obs_strictly_before_window_still_yields_zero_mean_err():
    ...
    feats = build_features(_baseline(), old, T0)
    assert (feats["mean_err_24h"] == 0.0).all()
    assert not feats.isna().any().any()
...
def test_assemble_stacks_one_issue_per_day():
    obs, baseline = _history(days=5)
    x, y = assemble(WAVE_STATION, obs, baseline, issue_hours=[6])
...
def test_assemble_multiple_issue_hours_gives_more_rows():
    obs, baseline = _history(days=5)
    x1, _ = assemble(WAVE_STATION, obs, baseline, issue_hours=[6])
    x2, _ = assemble(WAVE_STATION, obs, baseline, issue_hours=[6, 18])
...
def test_assemble_empty_history_returns_empty():
    ...
    x, y = assemble(WAVE_STATION, obs, baseline)
...
def test_assemble_rejects_unknown_kind():
    ...
    obs, baseline = _history()
    with pytest.raises(ValueError):
        assemble(bad, obs, baseline)
